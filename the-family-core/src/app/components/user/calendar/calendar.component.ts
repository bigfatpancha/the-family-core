...
export class CalendarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.usersService
      .doUserIdEventByDateGet(
        this.user.id,
        this.dateService.manageTimeZone(after.toISOString(), '0'),
        this.dateService.manageTimeZone(before.toISOString(), '0')
      )
      .subscribe(
        (res: EventResponse) => {
          const events: Event[] = this.eventRecurrence.allDatesFromRecurrence(
            res.results,
            after,
            before
          );
          this.calendarEvents = events.map((event: Event) => {
            const ev = new CalendarEventImpl();
  ...
  getAvatar(member: number) {
    const user = this.usersService.users.find(
      (user: FamilyUser) => user.id === member
    );
  ...
  dayClicked(event) {
    ...
    const offset = event.day.date.getTimezoneOffset()
      ? event.day.date.getTimezoneOffset() / 60
      : new Date().getTimezoneOffset() / 60;
    this.usersService
      .doUserIdEventByDateGet(
        this.user.id,
        this.dateService.manageTimeZoneAfter(after, offset),
        this.dateService.manageTimeZoneBefore(before, offset)
      )
      .subscribe(
        (res: EventResponse) => {
          const events: Event[] = res.results;
          this.spinner.hide();
          this.events = events;
          this.changeDetector.detectChanges();
        },
  ...
  edit(event: Event) {
    ...
    this.editRef.componentInstance.onEventPut.subscribe((event: Event) =>
  ...
  delete(event: Event) {
    this.spinner.show();
    this.usersService.doUserIdEventIdDelete(this.user.id, event.id).subscribe(
      () => {
        this.spinner.hide();
        alert('event deleted');
        this.dataChange(this.activeDay);
